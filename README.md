...
- 引导编辑 `site/topup+plans.json`，校验充值/套餐 JSON 格式
...
7. 提示在 `site/topup+plans.json` 中补充充值/套餐信息
8. 确认文件是合法 JSON 后启动本地服务并自动打开 `/panel/`
9. 保持终端驻留，等待继续新增或退出
...
`site/topup+plans.json` 用于人工维护充值/套餐信息，顶层按站点名分组。单条记录可以省略重复的“站点”和“倍率”，前端会用 `金额 / 实际额度` 推导倍率。
